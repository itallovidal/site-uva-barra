...
The system SHALL provide a Zod schema `newsSchema` in `src/schemas/news-schemas.ts` for validating news creation/editing form data. The inferred type SHALL NOT be used; the `useForm` generic SHALL use `NewsRequestDTO` from domain entities directly.
...
#### Scenario: Validates categoryId required
- **WHEN** the news schema validates without a categoryId
- **THEN** it SHALL produce a validation error

#### Scenario: Valid tagIds as string array
- **WHEN** the news schema validates tagIds
- **THEN** it SHALL accept an array of strings
